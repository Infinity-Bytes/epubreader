...
@synthesize resultsTableView, currentQuery, results, chapterDelegate, spineArrayManagerDelegate;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	[spineArrayManagerDelegate loadSpine:hit.chapterIndex atPageIndex:hit.pageIndex highlightSearchResult:hit];
}
...
- (void) searchString:(NSString *)query inChapterAtIndex:(int)index{
    ...
    Chapter* chapter = [[spineArrayManagerDelegate getCurrentSpineArray ]objectAtIndex:index];
    ...
    if(hitCount!=0){
        ...
        if((currentChapterIndex+1)<[[spineArrayManagerDelegate getCurrentSpineArray] count]){
            ...
            [spineArrayManagerDelegate setSearching:NO];
        }
    }
}
...
//    NSLog(@"w:%f h:%f", webView.bounds.size.width, webView.bounds.size.height);
    ...
    //NSString *setTextSizeRule = [NSString stringWithFormat:@"addCSSRule('body', '-webkit-text-size-adjust: %d%%;')", [[[spineArrayManagerDelegate getCurrentSpineArray] objectAtIndex:currentChapterIndex] ]];
